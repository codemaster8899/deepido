    ...
    <div class="right_col" role="main">
      <div class="col-xs-24">
        <ul class='breadcrumb pull-left'>
          <li><router-link :to="'/'"><i class="icon-home icon-black"></i></router-link></li>
          <li><router-link :to="'/devices'">{{ $t('main_nav.devices.title') }}</router-link></li>
          <li>{{ device.id }}</li>
        </ul>
      </div>
      ...
      <div v-if="device.status == 'shipped'" class="ro">
        <div class="col-xs-24 form styled-form" style="padding: 100px">
          <h3>{{ $t('devices.shipping_in_progress') }}</h3>
        </div>
      </div>
      ...
      <div v-else>
          ...
          <form v-on:submit.prevent="update" accept-charset="UTF-8" class="form styled-form" style="padding: 10px">
            <div class="form-group">
              ...
              <select v-model="device.status" class='form-control'>
                <option v-for="status in device.available_status" :value="status.name" :key="status.name">{{
                  $t(`device.${status.name}`) }}</option>
              </select>
            </div>
            ...
            <div class="form-group">
              <label>{{ $t('devices.manufacturer') }}*</label>
              <span class='error'>{{ errors.manufacturer }}</span>
              <select v-model="device.manufacturer_id" class='form-control'>
                <option v-for="manufacturer in device.manufacturers" :value="manufacturer.id" :key="manufacturer.id">{{
        ...
        <div class='col-xs-24 col-md-16'>
          ...
          <table class="table table-striped table-thin">
            <thead>
              <tr>
                <th style="width: 100px">{{ $t('devices.spot_id') }}</th>
