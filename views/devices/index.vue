    ...
    <div class="right_col" role="main">
      <div class="col-xs-24">
        <ul class="breadcrumb pull-left">
          <li>
            <router-link :to="'/'">{{ $t('main_nav.admin_title') }}</router-link>
          </li>
          <li>{{ $t('main_nav.devices') }}</li>
        </ul>
        <div class="pull-right">
          <router-link :to="'/devices/new'" class="add-link">+ {{ $t('devices.add_device') }}</router-link>
        </div>
      </div>

      <div class="col-xs-24 mt-10">
        <ul class="nav nav-tabs droppable">
          <li class="active"><router-link to="/devices">{{ $t('main_nav.devices') }}</router-link></li>
          <li><router-link to="/addons">{{ $t('devices.addons') }}</router-link></li>
            ...
            <table class="table table-striped table-thin">
              <thead>
                <tr>
                  ...
                  <th style="width: 100px;">{{ $t('devices.os') }}</th>
                  <th style="width: 200px;">{{ $t('devices.serial') }}</th>
                  <th style="width: 200px;">{{ $t('devices.model') }}</th>
                  <th>{{ $t('devices.localisation') }}</th>
                </tr>
              </thead>
              <tbody>
                <tr v-for="device in devices" :key="device.id">
                  ...
                  <td>
                    <span v-if="device.status == 'ready'">
                      <a @click.prevent="setShipped(device)" href="#">{{ $t(`devices.${device.status}`) }}</a>
                    </span>
                    <span v-else-if="device.status == 'returned'">
                      <a @click.prevent="modalRepair(device)" href="#">{{ $t(`devices.${device.status}`) }}</a>
                    </span>
                    <span v-else-if="device.status == 'to_repair'">
                      <a @click="modalRepair(device)" href="#">{{ $t(`devices.${device.status}`) }}</a>
                    </span>
                    <span v-else>{{ $t(`devices.${devices.status}`) }}</span>
                  </td>
                  <td>{{ $t(devices.os) }}</td>
                  <td>{{ device.serial }}</td>
