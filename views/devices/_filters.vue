    ...
    <form v-on:submit.prevent="filter" accept-charset="UTF-8" class="form col-xs-24 well hidden">
      ...
      <div class="form-group col-md-4">
        ...
        <select v-model="form.status_eq" class="form-control">
          <option value>{{ $t('all') }}</option>
          <option value="0">{{ $t('devices.stock') }}</option>
          <option value="1">{{ $t('devices.assigned') }}</option>
          <option value="2">{{ $t('devices.ready') }}</option>
          <option value="3">{{ $t('devices.shipped') }}</option>
          <option value="4">{{ $t('devices.active') }}</option>
          <option value="5">{{ $t('devices.to_repair') }}</option>
          <option value="6">{{ $t('devices.returned') }}</option>
          <option value="7">{{ $t('devices.out_of_order') }}</option>
        </select>
      </div>
      <div class="form-group col-md-4">
