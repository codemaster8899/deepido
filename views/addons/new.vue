    ...
    <div class="right_col" role="main">
      <div class="col-xs-24">
        <ul class="breadcrumb pull-left">
          ...
          <li>{{ $t('devices.new_addon') }}</li>
        </ul>
      </div>

      <div class="col-xs-24 mt-10">
        <ul class="nav nav-tabs droppable">
          <li><router-link to="/devices">{{ $t('main_nav.devices') }}</router-link></li>
          <li class="active"><router-link to="/addons">{{ $t('devices.addons') }}</router-link></li>
          <li><router-link to="/shapes">{{ $t('devices.model') }}</router-link></li>
        </ul>
...
export default {
  ...
  mounted: function () {
    this.$store.commit("AddonStore/one", {
      addon: {
        name: '',
        ref: '',
        price: 0
