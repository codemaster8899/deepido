            ...
            <table class="table table-striped table-thin table-hover">
              <thead>
                <tr>
                  ...
                  <th style="width: 120px;">{{ $t('created_at') }}</th>
                  <th style="width: 200px;">{{ $t('music.artist') }}</th>
                  <th>{{ $t('music.title') }}</th>
                  <th style="width: 130px;text-align: right">{{ $t('music.played_count') }}</th>
              ...
              <tbody>
                <tr v-for="content in contents" :key="content.id">
                  ...
                  <td>
                    {{ content.name }}
                    <br />
                    <a href="#" v-for="tag in content.tags" :key="tag.id" class="tagged"
                      :style="'background-color: ' + tag.color">{{ tag.label }}</a>
                  </td>
                  <td style="text-align: right">{{ content.played_count }}</td>
                  <td style="text-align: right"><input type="checkbox" :checked="content.validated" value="false"
                      @change="toggleValidated(content.id)" /></td>
                </tr>
              </tbody>
            </table>
          </div>
